...
- 🖥️ **控制台日志** - 程序会在控制台输出服务器启动信息、请求处理日志等（Debug 下如果没有控制台会自动创建）
- 👤 **用户管理** - 注册、登录、个人信息管理
- ✨ **智能推荐** - 基于个人营养目标生成每日餐单
- 📊 **营养追踪** - 实时查看营养摄入与目标的对比
- 📜 **历史记录** - 查看和管理历史餐单
...
- 首次运行会自动生成示例数据
- 按 Ctrl+C 可以停止服务器
- 本仓库当前主线为 **Web GUI 版本**。如果你想体验/找回早期的 **命令行交互菜单版本**，需要切换到历史分支（例如 `feat-intelligent-meal-recommender-cpp-vs2022`）再编译运行。
